...
class NbTestAdapter {
    constructor() {
        this.disposables = [];
        this.started = false;
        this.testController = vscode_1.tests.createTestController(constants_1.extConstants.ORACLE_VSCODE_EXTENSION_ID + '.testController', 'Java');
    ...
    load() {
        return __awaiter(this, void 0, void 0, function* () {
            for (let workspaceFolder of vscode_1.workspace.workspaceFolders || []) {
                const loadedTests = yield vscode_1.commands.executeCommand(commands_1.extCommands.loadWorkspaceTests, workspaceFolder.uri.toString());
                if (loadedTests) {
    ...
    run(request, cancellation) {
        return __awaiter(this, void 0, void 0, function* () {
            var _a, _b;
            if (!this.currentRun) {
                vscode_1.commands.executeCommand(commands_1.builtInCommands.focusReplDebug);
                ...
                if (request.include) {
                    ...
                    for (let item of include) {
                        if (item.uri) {
                            ...
                            if (!cancellation.isCancellationRequested) {
                                yield vscode_1.commands.executeCommand(((_a = request.profile) === null || _a === void 0 ? void 0 : _a.kind) === vscode_1.TestRunProfileKind.Debug ? commands_1.extCommands.debugTest : commands_1.extCommands.runTest, item.uri.toString(), idx < 0 ? undefined : item.id.slice(idx + 1));
                            }
                        }
                    }
                }
                    ...
                    for (let workspaceFolder of vscode_1.workspace.workspaceFolders || []) {
                        if (!cancellation.isCancellationRequested) {
                            yield vscode_1.commands.executeCommand(((_b = request.profile) === null || _b === void 0 ? void 0 : _b.kind) === vscode_1.TestRunProfileKind.Debug ? commands_1.extCommands.projectTestDebug : commands_1.extCommands.projectTest, workspaceFolder.uri.toString());
                        }
                    }
                }
                if (this.started) {
    ...
    updateTests(suite, testExecution) {
        ...
        const suiteUri = suite.file ? utils_1.FileUtils.toUri(suite.file, true) : undefined;
        ...
        const children = [];
        (_b = suite.tests) === null || _b === void 0 ? void 0 : _b.forEach(testCase => {
            var _a;
            let currentTest = currentSuite === null || currentSuite === void 0 ? void 0 : currentSuite.children.get(testCase.id);
            const testUri = testCase.file ? utils_1.FileUtils.toUri(testCase.file, true) : undefined;
            if (currentTest) {
                ...
                const testRange = (0, protocol_1.asRange)(testCase.range);
                ...
                if (!testExecution) {
                    currentTest = this.testController.createTestItem(testCase.id, testCase.name, testUri);
                    currentTest.range = (0, protocol_1.asRange)(testCase.range);
                    children.push(currentTest);
                    currentSuite === null || currentSuite === void 0 ? void 0 : currentSuite.children.add(currentTest);
                }
            }
    ...
    stacktrace2Message(currentTestUri, stacktrace) {
        ...
        for (const line of stacktrace) {
            ...
            if (result) {
                message.appendText(result[1]).appendText('(').appendMarkdown(`[${result[3]}](command:${commands_1.nbCommands.openStackTrace}?${encodeURIComponent(JSON.stringify([currentTestUri, result[2], result[4], +result[5]]))})`).appendText(')');
            }
            else {
...
//# sourceMappingURL=TestViewController.js.map